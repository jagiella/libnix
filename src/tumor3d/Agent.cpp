...
#include "PovrayIO.h"
#include <iostream>
void AgentList::printToPovray( const char *filename, VoronoiDiagram *vd)
{
	int histogram[10000];
	...
	for( int a=0; a<this->countActiveAgents; a++){
		...
		for( int l=0; l<this->agents[a]->countLocations; l++){
			...
			if( this->agents[a]->state == FREE){
				sprintf( color, "Yellow");
				sprintf( color_necrotic, "Yellow");
			}
			else
			if( this->agents[a]->state == NECROTIC){
				necrotic=true;
				sprintf( color, "Black");
				sprintf( color_necrotic, "Red");
			}
			else
			if( this->agents[a]->state == NONACTIVE
				//|| 0==vd->searchClosestFreeVoronoiCell( this->agents[a]->location[0], 20)
				)
				sprintf( color, "Blue");
			else
				if(this->agents[a]->state == ACTIVE){
					if(this->agents[a]->divide == 1){
						sprintf( color, "Red");
						dividing = true;
					}
					else
						sprintf( color, "NewMidnightBlue");
				}
			// NUCLEUS
			if( print){
			PovrayIO::writeSphere(&fs,
					this->agents[a]->location[l]->position[0],this->agents[a]->location[l]->position[1],
					NUCLEUS_DIAMETER/2. / SPATIAL_UNIT, color);
			PovrayIO::writeSphere(&fs_necrotic,
					this->agents[a]->location[l]->position[0],this->agents[a]->location[l]->position[1],
					NUCLEUS_DIAMETER/2. / SPATIAL_UNIT, color_necrotic);
			// CELL
			PovrayIO::writeSphere(&fs,
					this->agents[a]->location[l]->position[0],this->agents[a]->location[l]->position[1], -1,
					AGENT_DIAMETER/2. / SPATIAL_UNIT, "White transmit 0.7");
			PovrayIO::writeSphere(&fs_necrotic,
					this->agents[a]->location[l]->position[0],this->agents[a]->location[l]->position[1], -1,
					AGENT_DIAMETER/2. / SPATIAL_UNIT, "White transmit 0.7");

			// NEIGHBORS
			for( int n=0; n<this->agents[a]->location[l]->countNeighborCells; n++)
				if(this->agents[a]->location[l]->neighborCells[n]->getState() == FREE)
				PovrayIO::writeSphere(&fs,
						this->agents[a]->location[l]->neighborCells[n]->position[0],this->agents[a]->location[l]->neighborCells[n]->position[1], -1,
						AGENT_DIAMETER/2. / SPATIAL_UNIT, "White transmit 0.7");
			}
		...
		if( VoronoiCell::USE_SYMBOLIC_EXTENDED_NEIGHBORHOOD)
			border = vd->searchClosestFreeVoronoiCell(
					this->agents[a]->location[0],
					VoronoiCell::extendedNeighborDepth,
					VoronoiCell::symbolicExtendedNeighborhoodSize,
					VoronoiCell::symbolicExtendedNeighborhood);
		else
			border = vd->searchClosestFreeVoronoiCell( this->agents[a]->location[0], 100);
		if( border){
			double dist = border->getDistanceTo( this->agents[a]->location[0]) * SPATIAL_UNIT;
			for( int i=(dist-AGENT_DIAMETER>0.?(int)(dist-AGENT_DIAMETER):0); i<(int)(dist); i++){
				histogram[i]++;
				if(dividing)
					histogramDividing[i]++;
				if(necrotic)
	...
	for( int i=0; i<10000; i++)
		if(histogram[i]){
			fs <<  i << " "
			   << histogram[i] << " "
			   << histogramDividing[i] << " "
			   << (double)histogramDividing[i]/(double)histogram[i] << " "
			   << histogramNecrotic[i] << " "
			   << (double)histogramNecrotic[i]/(double)histogram[i] << " "
			   << std::endl;
