...
//			perror( "Error reading line" );
	...
	/*this->countTetrahedra = tetrahedra = t;
	fprintf( stderr, "Read %i tets\n", tetrahedra);*/
 ...
 ****************************************************************************/
	if( fgets( buffer, READBUFFERSIZE, fp_tet ) != NULL ){
		...
		//perror( "WARNING: Simplex file doesn't contain information about neighborship relations of simplices" );
	}
 ...
 ****************************************************************************/
	...
	for( i = 0; i<lastpoint; i++ ){
#if _COMMENTS_ > 2
		fprintf( stderr, "\rInitialize VoronoiDiagram %.3lf%%                              \b", 100.*(i+1.)/this->countVoronoiCells);
#endif		
	  ...
	  //printf("initialize point %i\n", i);
		...
		//this->voronoiCells[i].free_neighbors = point[ i ][ 0 ] - 1;
	...
	for( int i=0; i<this->countVoronoiCells; i++){
		...
		for( int ii=0; ii<this->voronoiCells[i]->countNeighborCells; ii++){
			if( abs( (int)(this->voronoiCells[i]->position[0]) - (int)(this->voronoiCells[i]->neighborCells[ii]->position[0])) < LIMITED_NEIGHBOR_DISTANCE
#if DIMENSIONS >= 2
			    && abs( (int)(this->voronoiCells[i]->position[1]) - (int)(this->voronoiCells[i]->neighborCells[ii]->position[1])) < LIMITED_NEIGHBOR_DISTANCE
#endif
#if DIMENSIONS == 3
			    && abs( (int)(this->voronoiCells[i]->position[2]) - (int)(this->voronoiCells[i]->neighborCells[ii]->position[2])) < LIMITED_NEIGHBOR_DISTANCE
#endif
			){
			
				//countReachable++;
				this->voronoiCells[i]->neighborCells[countReachable++] = this->voronoiCells[i]->neighborCells[ii];
			}
		}
		
		this->voronoiCells[i]->countNeighborCells     = countReachable;
		this->voronoiCells[i]->countFreeNeighborCells = countReachable;
	}
#endif		

	/*for( i = 0; i<lastpoint; i++ ){
		printf( "point %i: ", this->voronoiCells[i].index);
		for( j = 0; j < this->voronoiCells[i].countNeighborCells; j++)
			printf( " %i", this->voronoiCells[i].neighborCells[j]->index);
		printf( "\n");
	}*/
...
void printToPovray( const char *filename, VoronoiDiagram *voronoiDiagram, VoronoiCell **newCell, int countNewCells, int printPoints, int printNeighborship, int printTetrahedra, int printConvexHull)
{
	...
	fprintf( fp, "#include \"finish.inc\"\n"\
				...
				"camera {location <1.5, 1.5, 10> look_at  <1.5, 1.5, 0>}\n"\
				"light_source {<5, 5, 12> color rgb <1, 1, 1>}\n");

	// all points (GRAY)
	for( i=0; i<voronoiDiagram->countVoronoiCells; i++)
	if( (int)(voronoiDiagram->xMin[2]+voronoiDiagram->xMax[2])/2 == (int)voronoiDiagram->voronoiCells[i]->position[2] )
	{
		if( printPoints){
		fprintf( fp, "sphere {<");
		for( j=0; j<DIMENSIONS; j++){
		...
		for( j=0; j<voronoiDiagram->voronoiCells[i]->countNeighborCells; j++){
			fprintf( fp, "  cylinder {<%lf, %lf, %lf>, <%lf, %lf, %lf>, %lf pigment { color %s }}\n",
				...
				voronoiDiagram->voronoiCells[i]->position[2],
	       		voronoiDiagram->voronoiCells[i]->position[0]
				+ 0.4 * (voronoiDiagram->voronoiCells[i]->neighborCells[j]->position[0] - voronoiDiagram->voronoiCells[i]->position[0]),
	       		voronoiDiagram->voronoiCells[i]->position[1]
				+ 0.4 * (voronoiDiagram->voronoiCells[i]->neighborCells[j]->position[1] - voronoiDiagram->voronoiCells[i]->position[1]),
				voronoiDiagram->voronoiCells[i]->position[2]
				+ 0.4 * (voronoiDiagram->voronoiCells[i]->neighborCells[j]->position[2] - voronoiDiagram->voronoiCells[i]->position[2]),
				0.05,
	           	"Yellow");
			fprintf( fp, "\n");
		}
